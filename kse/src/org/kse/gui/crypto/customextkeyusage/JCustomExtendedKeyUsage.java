...
//		sorter.setComparator(0, new CustomExtKeyUsageTableModel.CustomExtKeyUsageComparator());
		...
		for (int i = 0; i < jtCustomExtKeyUsages.getColumnCount(); i++) {
			...
			column.setHeaderRenderer(new CustomExtKeyUsageTableHeadRend(jtCustomExtKeyUsages.getTableHeader()
					.getDefaultRenderer()));
	 ...
	 */
	public Set<ASN1ObjectIdentifier> getCustomExtKeyUsage() {
		return objectIds;
	}
	 ...
	 *            Policy information
	 */
	public void setCustomExtKeyUsage(Set<ASN1ObjectIdentifier> customExtKeyUsage) {
		this.objectIds = customExtKeyUsage;
		populate();
	}
	...
	private void removeSelectedCustomExtKeyUsage() {
		...
		if (selectedRow != -1) {
			ASN1ObjectIdentifier extKeyUsageOid = (ASN1ObjectIdentifier) jtCustomExtKeyUsages.getValueAt(selectedRow, 0);
	...
	private void selectCustomExtKeyUsageInTable(ASN1ObjectIdentifier policyInfo) {
		for (int i = 0; i < jtCustomExtKeyUsages.getRowCount(); i++) {
			if (policyInfo.equals(jtCustomExtKeyUsages.getValueAt(i, 0))) {
